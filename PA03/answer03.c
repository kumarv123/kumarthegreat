...
int * readIntegers(const char * filename, int * numberOfIntegers)
{
    ...
    int ind = 0;
    int *arr;
    fptr = fopen(filename,"r"); //This opens the file in reading mode
    if(fptr == NULL) //This check if if file is NULL
    {
	fclose(fptr);
	return NULL;
    }
    
    while(fscanf(fptr,"%d",&temp) == 1) //Checks for integer in a linear
    {
	count++;// Counts the number of integers
    }
    
    arr = malloc(sizeof(int)*count); //Allocates memory
    
    fseek(fptr,0,SEEK_SET); //Points the pointer to the the start of the file
   
    while(fscanf(fptr,"%d",&temp) == 1) //Checks for integer in a linear
    {
	
	arr[ind] = temp;
	ind++;// Counts the number of integers
    }
...
{  
    ...
    {
	high = mid -1;
	return helper(arr,low,high,key);
    }
...
int search(int * arr, int length, int key)
{
    ...
    return helper(arr,0,length-1,key);
